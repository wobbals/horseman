...
function onBFC(e) {
  debug("onBeginFramesChanged");
  debug(e);
}

function onBFScreenshotsReady(e) {
  debug("onBFScreenshotsReady");
  debug(e);
}

function onTargetAttached(e) {
  debug("onTargetAttached");
  debug(e);
}

function onTargetCreated(e) {
  debug("onTargetCreated");
  debug(e);
}
const setupRemoteHooks = async function(remoteChrome) {
  ...
  remoteChrome.on("Target.attachedToTarget", onTargetAttached);
  remoteChrome.on("Target.targetCreated", onTargetCreated);
}

const initScreencastBF = async function(url) {
  try {
    let browserContext = await Target.createBrowserContext();
    let browserContextId = await browserContext.browserContextId;
    ...
    debug(`BF: target ${target.targetId} is focused`);
    await HeadlessExperimental.enable(); // how to associate with target?
    ...
    debug("HEY LOOK A BeginFrame! ", bf);
  } catch (e) {
