...
function onPageScreencast(e) {
  ...
  if (pageForceEvent &&
    ...
    lastFreshFrameTime = new Date();
  }
  if (lastFreshFrameTime && (new Date() - lastFreshFrameTime) > 300000) {
...
function onBFC(e) {
  ...
  if (e.needsBeginFrames) {
    beginFrameTimer = setInterval(sendBeginFrame, 67);
...
const sendBeginFrame = async function() {
  try {
    ...
    if (bf.mainFrameContentUpdated && bf.screenshotData && eventHandlers.screencast) {
